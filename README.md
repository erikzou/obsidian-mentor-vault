...
一個 Obsidian vault + AI agent 工作流。你提供 mentor 的原始內容（書、Podcast、文章），agent 自動整理成結構化筆記。之後你可以隨時提問，agent 依據 mentor 的真實思想框架，從整理好的資料中取材回答。

## 設計理念

- **真實性優先** — 回答基於 mentor 的實際著作，附出處連結，絕不編造
...
3. Agent 自動建立 mentor 資料、主題索引、摘要與原文引述
...
Mentors/
    {mentor-slug}/
        mentor-profile.md       # Mentor 身份、框架、資料清單
...
- **事實層** — mentor 的原話與觀點，附出處連結
- **推論層** — 基於 mentor 框架分析你的具體情境
- **延伸層** — 超出 mentor 範圍的補充，標記為 AI 判斷
...
告訴 agent「新增 mentor」並提供名稱，agent 會建立資料夾結構。再提供內容即可。
...
Vault 架構與工作流為開源。Mentor 的書籍內容受各自版權保護，不包含在 repo 中。
